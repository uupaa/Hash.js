...
})("Hash", function moduleClosure(global) {
"use strict";
...
var Hash = {
    ...
    "CRC32":        CRC32,      // Hash.CRC32(source:Uint8Array|String, hex:Boolean = false, offset:UINT32 = 0, length:UINT32 = source.length, crc:UINT32 = 0):UINT32|HexString
    "CRC32M":       CRC32M,     // Hash.CRC32M(source:Uint8Array|String, hex:Boolean = false, offset:UINT32 = 0, length:UINT32 = source.length, crc:UINT32 = 0):UINT32|HexString
    "HexDump":      HexDump,    // Hash.HexDump(source:Uint8Array|Uint8ArrayArray, options:Object = {}):void
...
// === CRC32M ==============================================
var CRC32M_TABLE = null;

function CRC32M(source, // @arg Uint8Array|String
                hex,    // @arg Boolean = false
                offset, // @arg UINT32 = 0
                length, // @arg UINT32 = source.length
                crc) {  // @arg UINT32 = 0 - initial value
                        // @ret UINT32|HexString
    if (!CRC32M_TABLE) {
        CRC32M_TABLE = new Uint32Array(256);
        _createCRC32MTable();
    }
    ...
    for (; i < iz; ++i) {
        c[0] = (c[0] << 8) ^ CRC32M_TABLE[((c[0] >> 24) ^ source_[i]) & 0xff];
    }
    return hex ? toHexString(c[0]) : c[0];
}

function _createCRC32MTable() {
    var c = new Uint32Array(3); // c[0] = crc, c[1] = i, c[2] = j;
    for (c[1] = 0; c[1] < 256; ++c[1]) {
        c[0] = 0;

        for (c[2] = (c[1] << 24) | 0x800000; c[2] !== 0x80000000; c[2] <<= 1) {
            c[0] = (c[0] << 1) ^ (((c[0] ^ c[2]) & 0x80000000) ? 0x04c11db7 : 0);
        }
        CRC32M_TABLE[c[1]] = c[0];
    }
}
...
function CRC32M(source, // @arg Uint8Array|String
    ...
    for (; i < iz; ++i) {
        ...
        for (var k = 0; k < 8; ++k) {
            c[0] = (c[0] & 0x80000000) ? (c[0] << 1) ^ 0x04c11db7
                                       : (c[0] << 1);
        }
    }
